...
def _parse_block(name, block):
    ...
    if pm:
        ...
        for nm in re.finditer(r"nodes = \(([^)]*)\);", block[s:e]):
            nodes = [[_num(x), _num(y), t, bool(sm)]
                     for x, y, t, sm in NODE_RE.findall(nm.group(1))]
            glyph["paths"].append({"closed": 1, "nodes": nodes})
    return glyph
...
def _serialize_paths(paths):
    ...
    for p in paths:
        lines = ",\n".join(
            f'"{_num(x):g} {_num(y):g} {t}{" SMOOTH" if sm else ""}"'
...
def export_js(out_path):
    out_path.write_text(
        ...
        "window.KAWARA_GLYPHS = " + json.dumps(payload(), indent=1, sort_keys=True) + ";\n"
    )
    n = len(payload()["order"])
